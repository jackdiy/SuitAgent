...
SuitAgent 是一个基于 **Claude Code 架构** 的诉讼法律服务智能分析系统。它采用10个专业AI代理协作的模式，将复杂的诉讼案件分析分解为多个可独立执行的工作流，实现法律文书的工程化生成。
...
**方式1：使用Zed（推荐）**：
- 在Zed左侧文件树中展开 `output/cases/[案件编号]/`
- 双击文件即可查看
- 文件树实时更新，新生成的文件会自动显示
...
```bash
# macOS
open output/cases/[案件编号]/

# Windows
start output/cases\[案件编号]\

# Linux
xdg-open output/cases/[案件编号]/
```
